...
compile0:
	...
	nvcc -o ./build/opt0/RBParallel_CUDA_OpenMP0 ./src/RBParallel_CUDA_OpenMP.cu ./src/RBMatrix.c -Xcompiler -fopenmp -O0
compile1:
	...
	nvcc -o ./build/opt1/RBParallel_CUDA_OpenMP1 ./src/RBParallel_CUDA_OpenMP.cu ./src/RBMatrix.c -Xcompiler -fopenmp -O1
compile2:
	...
	nvcc -o ./build/opt2/RBParallel_CUDA_OpenMP2 ./src/RBParallel_CUDA_OpenMP.cu ./src/RBMatrix.c -Xcompiler -fopenmp -O2
compile3:
	...
	nvcc -o ./build/opt3/RBParallel_CUDA_OpenMP3 ./src/RBParallel_CUDA_OpenMP.cu ./src/RBMatrix.c -Xcompiler -fopenmp -O3
...
test03:
	...
	@for mpiProc in $(MPI_PROCESSES); do \
        for ompThread in $(THREADS); do \
            if [ $$((mpiProc * ompThread)) -le $(MAX_THREADS) ]; then \
                for numValues in $(VALS2); do \
                    mpiexec -np $${mpiProc} ./build/opt3/RBParallel_MPI_OpenMP3 $${ompThread} $${numValues} $(SEED2) ./data/MPIOpenMPTestResult/opt3/test_MPI_OpenMP_$${mpiProc}_$${ompThread}_$${numValues}.txt ./data/MPIOpenMPCSVResult/opt3/csv_MPI_OpenMP_$${mpiProc}_$${ompThread}_$${numValues}.csv; \
                done \
            fi \
        done \
    done
